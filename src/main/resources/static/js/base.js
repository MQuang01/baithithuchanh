...
function createOptionRadio(props) {
    const radios = props.options.map(option => `<input type="radio" name="${props.name}" value="${option.value ? '1' : '0'}" ${props.value ? 'checked' : ''}>
                                                    <label>${option.name}</label>`).join('');
    return `<div class="${props.classContainer || ''}">
                    <label class="${props.classLabel || ''} form-label">${props.label}</label>
                    </br>${radios}</br>
                    <span class="error form-text ${props.classError}">${props.message}</span>
...
function createFieldForm(props) {
    ...
    if (props.type === 'radio') {
        return createOptionRadio(props);
    }
